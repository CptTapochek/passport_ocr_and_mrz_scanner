...
import 'package:passport_nfc_check/camera_screen.dart';
...
String formatMRZ(final MRZD mrz) {
  return "MRZ\n"
      "  version: ${mrz.version}\n"         +
...
class _MrtdHomePageState extends State<MrtdHomePage> {
  ...
  bool isParsed = false;
  MRZController? _scannerController;
  ...
  void initState() {
    ...
    _timerStateUpdater = Timer.periodic(const Duration(seconds: 10), (Timer t) {
      _initPlatformState();
    });
  }

  void pictureScan(result) async {
    List dobArray = result.birthDate.toString().replaceAll(" 00:00:00.000", "").split("-");
    List doeArray = result.expiryDate.toString().replaceAll(" 00:00:00.000", "").split("-");

    setState((){
      _docNumber.text = result.documentNumber;
      _dob.text = "${dobArray[1]}/${dobArray[2]}/${dobArray[0]}";
      _doe.text = "${doeArray[1]}/${doeArray[2]}/${doeArray[0]}";
    });
  }
  ...
  Image getPassportImage({required String hexCode}) {
    List infoList = hexCode.replaceAll("7f61", "-7f61").replaceAll("7f60", "-7f60").replaceAll("5f2e", "-5f2e").replaceAll("7F2E", "-7F2E").replaceAll("ffd8", "-ffd8").split("-");
    final String identificator = infoList[0];
    final String instances = infoList[1];
    final String header = infoList[2];
    final String facePicture = infoList[4];

    Image image;
    String hexPic = facePicture;
    String input = hexPic.replaceAll("/[^A-Fa-f0-9]/g", "");
    if (input.length.isOdd) {
      /* Incorrect hex code */
      print("ERROR!!! cleaned hex string length is odd.");
      // return;
    }
    List<int> binary = [];
    List<int> emptyFields = List.filled(input.length, 0);
    binary.addAll(emptyFields);
    for (int idx = 0; idx < input.length / 2; idx++) {
      var h = input.substring(idx * 2, idx * 2 + 2);
      binary[idx] = int.parse(h, radix: 16);
    }
    var byteArray = Uint8List.fromList(binary);
    image = Image.memory(byteArray, width: 160);
    ...
    if (_mrtdData!.dg2 != null) {
      ...
      list.add(
        Container(
          ...
          child: getPassportImage(hexCode: _mrtdData!.dg2!.toBytes().hex()),
        ));
    }
                  ...
                  child: SingleChildScrollView(
                      ...
                      child: Column(
                          ...
                          children: <Widget>[
                            ...
                            _buildForm(context),
                            const SizedBox(height: 20),
                            PlatformTextButton( // btn Read MRTD
                            ...
                            TextButton(
                              onPressed: () => Navigator.push(
                                context,
                                MaterialPageRoute(builder: (context) => CameraScreen(callBack: pictureScan, context: context)),
                              ),
                              child: Container(
